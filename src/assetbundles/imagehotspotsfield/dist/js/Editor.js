...
Born05.ImageHotspotEditor = Garnish.Modal.extend(
    {
        ...
        init: function (asset, pos, listData, settings) {
            ...
            // Build the modal
            this.$container = $('<div class="modal fitted image-hotspot-editor"></div>').appendTo(
                Garnish.$bod,
            );
            ...
            this.$imgContainer = $('<div class="image-hotspot-container"></div>').appendTo(
                this.$body,
            );
            this.$primaryButtons = $('<div class="image-hotspot-buttons"/>').appendTo(this.$body);
            this.$saveBtn = $(
                '<div class="btn submit">' + Craft.t('app', 'Done') + '</div>',
            ...
            this.addListener(this.$container, 'mousedown,touchstart', 'handleDragStart');
            this.addListener(this.$container, 'mousemove,touchmove', 'handleDragMove');
            this.addListener(this.$container, 'mouseup,touchend,touchcancel', 'handleDragEnd');
        },
        ...
        onShow: function () {
            ...
            Object.keys(this.listData).forEach(function (key) {
                ...
                var $label = $(`<span></span>`).appendTo($point);

                if (this.listData[key].index !== null && this.listData[key].label !== null) {
                    $label.text(`${this.listData[key].index}: ${this.listData[key].label}`);
                } else if (this.listData[key].label !== null) {
                    $label.text(this.listData[key].label);
                } else if (this.listData[key].index !== null) {
                    $label.text(this.listData[key].index);
                }
                ...
                this.$imgContainer.css({
                    ...
                    left:
                        (this.containerBounds.width -
                            this.asset.width * (this.containerBounds.height / this.asset.height)) /
                        2,
                ...
                this.$imgContainer.css({
                    ...
                    top:
                        (this.containerBounds.height -
                            this.asset.height * (this.containerBounds.width / this.asset.width)) /
                        2,
                    left: 0,
                    transform: 'translate(0, 0)',
                    opacity: 1,
